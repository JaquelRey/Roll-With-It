...
db.once('open', async () => {
	...
	const marris = await Character.create({
		character:{
	...
	const taro = await Character.create({
		character:{
		  ...
		  // idk trying anything lol
	User.pre('save', function(next) {
		let doc = this;     
		let char = marris    
	  
		User.findOneAndUpdate({ email: "pamela@testmail.com" },
			{ $addToSet: { characters: char } }, function(error,data) {         
		  if(error)
			  next(error)               
